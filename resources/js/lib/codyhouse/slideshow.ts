...
Slideshow.prototype.showItem = function (index: any) {
...
function initSlideshow(slideshow: any) {
...
function initSlideshowEvents(slideshow: any) {
    // if slideshow navigation is on -> create navigation HTML and add event listeners
    if (slideshow.options.navigation) {
        // check if navigation has already been included
        if (slideshow.element.getElementsByClassName('js-slideshow__navigation').length == 0) {
            let navigation = document.createElement('ol'),
                navChildren = '';

            let navClasses = slideshow.options.navigationClass + ' js-slideshow__navigation';
        ...
        dotsNavigation.addEventListener('click', function (event: any) {
            navigateSlide(slideshow, event, true);
        });
        dotsNavigation.addEventListener('keyup', function (event: any) {
    ...
    if (slideshow.controls.length > 0) {
        ...
        slideshow.controls[0].addEventListener('click', function (event: any) {
            event.preventDefault();
            slideshow.showPrev();
            updateAriaLive(slideshow);
        });
        slideshow.controls[1].addEventListener('click', function (event: any) {
            event.preventDefault();
            slideshow.showNext();
            updateAriaLive(slideshow);
        });
    ...
    if (slideshow.options.swipe) {
        ...
        slideshow.element.addEventListener('swipeLeft', function (event: any) {
            slideshow.showNext();
        });
        slideshow.element.addEventListener('swipeRight', function (event: any) {
            slideshow.showPrev();
        });
    }
    // autoplay
    if (slideshow.options.autoplay) {
        ...
        if (!slideshow.options.autoplayOnHover) {
            slideshow.element.addEventListener('mouseenter', function (event: any) {
                slideshow.pauseAutoplay();
                slideshow.autoplayPaused = true;
            });
            slideshow.element.addEventListener('mouseleave', function (event: any) {
                slideshow.autoplayPaused = false;
                slideshow.startAutoplay();
            });
        }
        if (!slideshow.options.autoplayOnFocus) {
            slideshow.element.addEventListener('focusin', function (event: any) {
                slideshow.pauseAutoplay();
                slideshow.autoplayPaused = true;
            });
            slideshow.element.addEventListener('focusout', function (event: any) {
                slideshow.autoplayPaused = false;
                slideshow.startAutoplay();
            });
        }
    ...
    slideshow.element.addEventListener('selectNewItem', function (event: any) {
    ...
    // keyboard navigation
    slideshow.element.addEventListener('keydown', function (event: any) {
...
function navigateSlide(slideshow: any, event: any, keyNav: any) {
...
function initAnimationEndEvents(slideshow: any) {
...
function resetAnimationEnd(slideshow: any, item: any) {
    setTimeout(function () {
...
function showNewItem(slideshow: any, index: any, bool: any) {
...
function getExitItemClass(slideshow: any, bool: any, oldIndex: any, newIndex: any) {
    let className = '';
    if (bool) {
        className =
            bool == 'next'
...
function getEnterItemClass(slideshow: any, bool: any, oldIndex: any, newIndex: any) {
    let className = '';
    if (bool) {
        className =
            bool == 'next' ? 'slideshow__item--' + slideshow.animationType + '-in-right' : 'slideshow__item--' + slideshow.animationType + '-in-left';
    } else {
        className =
            newIndex < oldIndex
...
function resetSlideshowNav(slideshow: any, newIndex: any, oldIndex: any) {
...
function resetSlideshowTheme(slideshow: any, newIndex: any) {
...
function emitSlideshowEvent(slideshow: any, eventName: any, detail: any) {
    const event = new CustomEvent(eventName, { detail: detail });
    slideshow.element.dispatchEvent(event);
}

function updateAriaLive(slideshow: any) {
    slideshow.ariaLive.innerHTML = 'Item ' + (slideshow.selectedSlide + 1) + ' of ' + slideshow.items.length;
}

function externalControlSlide(slideshow: any, button: any) {
