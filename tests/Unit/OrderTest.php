...
test('can create an order with discounted items', function () {
...
test('can create order items with discounts', function () {
    ...
    expect($order->orderItems[0]->has_discount)->toBe($cart->items[0]->has_discount);
    expect($order->orderItems[0]->total)->toBe($cart->items[0]->total);
    expect($order->orderItems[0]->total_with_taxes)->toBe($cart->items[0]->total_with_taxes);
    expect($order->orderItems[0]->computed_taxes)->toBe($cart->items[0]->computed_taxes);
    expect($order->orderItems[0]->price)->toBe($cart->items[0]->price);
    ...
    expect($order->orderItems[1]->has_discount)->toBe($cart->items[1]->has_discount);
    expect($order->orderItems[1]->discounted_price)->toBe($cart->items[1]->discounted_price);
...
test('can add a new order item with discount', function () {
    ...
    expect($order->orderItems[2]->has_discount)->toBe($newItem->has_discount);
...
test('can update an order item with discount', function () {
    ...
    $item = CartItem::factory()->create([
        ...
        'total_with_taxes' => $discountedPrice * 2 * (1 + 0.15),
        'computed_taxes' => $discountedPrice * 2 * 0.15,
    ...
    $item->quantity = $newQuantity;
    $item->total = $newQuantity * $discountedPrice;
    $item->total_with_taxes = $newQuantity * $discountedPrice * (1 + 0.15);
    $item->computed_taxes = $item->total_with_taxes - $item->total;
